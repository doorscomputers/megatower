...
import { Droplet, Save, Calendar, AlertCircle, Info } from "lucide-react"
...
export default function WaterReadingsPage() {
  ...
  const loadExistingReadings = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/readings/water?billingPeriod=${billingPeriod.toISOString()}`
      )
      ...
      for (const unit of floorUnits) {
        ...
        if (existing) {
          ...
          const lastRes = await fetch(
            `/api/readings/water?unitId=${unit.id}`
          )
  ...
  const handleSaveAll = async () => {
    ...
    const invalidReadings = unitsWithReadings.filter((r) => r.consumption < 0)
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        {/* Header with Water theme (Blue/Cyan) */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-cyan-100">
                Enter monthly water meter readings by floor
              </p>
            </div>
          </div>
        </div>
        ...
        <Card className="border-cyan-200">
          <CardHeader className="bg-cyan-50">
            <CardTitle className="text-cyan-800">Select Building, Floor and Billing Month</CardTitle>
          </CardHeader>
        ...
        {selectedFloor && billingMonth && filteredUnits.length > 0 && (
          <Card className="border-cyan-200">
            <CardHeader className="flex flex-row items-center justify-between bg-cyan-50">
              <div>
              ...
              <div className="overflow-auto max-h-[calc(100vh-400px)] min-h-[300px]">
                <table className="w-full">
                  <thead className="sticky top-0 z-10 shadow-sm">
                    <tr className="border-b bg-cyan-100">
                      <th className="text-left p-3 font-medium text-cyan-900 bg-cyan-100">Unit</th>
                      <th className="text-left p-3 font-medium text-cyan-900 bg-cyan-100">Owner</th>
                      <th className="text-right p-3 font-medium text-cyan-900 bg-cyan-100">Previous</th>
                      <th className="text-right p-3 font-medium text-cyan-900 bg-cyan-100">Present</th>
                      <th className="text-right p-3 font-medium text-cyan-900 bg-cyan-100">
                        Consumption (cu.m)
                      </th>
                      <th className="text-left p-3 font-medium text-cyan-900 bg-cyan-100">Remarks</th>
                    </tr>
                  </thead>
                  <tbody>
                  ...
                  <tfoot>
                    <tr className="bg-cyan-100 font-semibold">
                      <td colSpan={4} className="p-3 text-right text-cyan-900">
                        Total Consumption:
                      </td>
        ...
        {!selectedFloor || !billingMonth ? (
          <Card className="border-cyan-200">
            <CardContent className="flex flex-col items-center justify-center py-12 bg-cyan-50">
              <Droplet className="h-12 w-12 text-cyan-400 mb-4" />
