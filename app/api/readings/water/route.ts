...
// GET /api/readings/water - Get water readings
...
// POST /api/readings/water - Create or update water reading
export async function POST(request: NextRequest) {
  try {
    ...
    const lastReading = await prisma.waterReading.findFirst({
      where: {
        unitId,
        billingPeriod: {
    ...
    if (existing) {
      // Update existing
      reading = await prisma.waterReading.update({
        where: {
          id: existing.id,
        },
        data: {
      ...
      // Create new
      reading = await prisma.waterReading.create({
        data: {
          unitId,
          billingPeriod: billingPeriodDate,
          readingDate: new Date(),
